...
ENV NEXT_PUBLIC_YAHOO_OPTIONS_URL=api/yahoo-finance-stock-options
...
# Process-liveness check (returns 200 from the SPA fallback even if the
# upstream API is broken — this only verifies nginx is serving, not the API).
